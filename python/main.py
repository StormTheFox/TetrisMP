...
def save_custom_alg_model(name: str, params: dict, tags=None) -> int:
    name = str(name or "").strip()
    if not name:
        raise ValueError("Имя модели не задано.")
...
class AuthWindow:
    ...
    def build_register(self):
        ...
        tk.Label(f, text="Игровой ник по умолчанию:", bg="#111", fg="#eee").pack(pady=(10, 5))
        self.reg_default_nick = tk.Entry(f, bg="#333", fg="#fff", insertbackground="#fff")
        self.reg_default_nick.pack()

        tk.Label(f, text="Любимый цвет:", bg="#111", fg="#eee").pack(pady=(10, 5))
        self.reg_color = "#FF4444"
        self.reg_color_btn = tk.Button(
            f,
            text=self.reg_color,
            bg=self.reg_color,
            fg="white",
        ...
        tk.Label(f, text="Стандартная скорость:", bg="#111", fg="#eee").pack(pady=(10, 0))
        self.reg_speed = tk.DoubleVar(value=5.0)
        self.reg_speed_label = tk.Label(
            f,
            text="5.0",
            bg="#111",
            fg="#eee",
        ...
        tk.Scale(
            ...
            resolution=0.1,
            orient="horizontal",
            variable=self.reg_speed,
            bg="#111",
            fg="#e94560",
            troughcolor="#333",
            length=240,
            showvalue=0,
            command=lambda v: self.reg_speed_label.config(text=f"{float(v):.1f}"),
        ).pack(pady=(0, 10))
    ...
    def pick_reg_color(self):
        color = colorchooser.askcolor(
            title="Любимый цвет игрока",
            color=self.reg_color,
        )
...
class MainMenuWindow:
    def __init__(self, root, username, on_play, on_leaderboard, on_logout):
        Log.info(f"🎮 Инициализация главного меню для пользователя: {username}")
        self.root = root
        ...
        try:
            ...
            tk.Button(
                ...
                bg="#533483",
                fg="#fff",
                command=self._handle_leaderboard,
                **btn_style,
            ).pack(pady=8)
            ...
            Log.info("✅ Главное меню успешно создано")
            
        except Exception as e:
            Log.error(f"❌ Ошибка при создании главного меню: {e}")
            messagebox.showerror("Ошибка", f"Не удалось создать меню: {e}")

    def _handle_play(self):
        """Обработчик кнопки 'Играть' с защитой от исключений"""
        Log.info(f"▶ Пользователь {self.username} нажал 'Играть'")
        try:
            if callable(self.on_play):
                self.on_play()
        except Exception as e:
            Log.error(f"❌ Ошибка при запуске игры: {e}")
            messagebox.showerror("Ошибка", f"Не удалось запустить игру: {e}")

    def _handle_leaderboard(self):
        """Обработчик кнопки 'Таблица лидеров' с защитой от исключений"""
        Log.info(f"🏆 Пользователь {self.username} открыл таблицу лидеров")
        try:
            if callable(self.on_leaderboard):
                self.on_leaderboard()
        except Exception as e:
            Log.error(f"❌ Ошибка при открытии таблицы лидеров: {e}")
            messagebox.showerror("Ошибка", f"Не удалось открыть таблицу: {e}")

    def _handle_logout(self):
        """Обработчик кнопки 'Выйти' с защитой от исключений"""
        Log.info(f"🚪 Пользователь {self.username} выходит из аккаунта")
        try:
            if callable(self.on_logout):
                self.on_logout()
        except Exception as e:
            Log.error(f"❌ Ошибка при выходе из аккаунта: {e}")
            messagebox.showerror("Ошибка", f"Не удалось выйти: {e}")
    ...
    def load_data(self):
        ...
        for i, row in enumerate(rows, 1):
            ...
            self.tree.insert(
                "",
                "end",
                values=(i, user, score, bot_str, short_date),
            )
...
class CustomAISettings(tk.Toplevel):
    ...
    def build_ui(self):
        ...
        left = tk.LabelFrame(
            main,
            text="Параметры (значения можно вводить вручную)",
            bg="#111",
            fg="#e94560",
        ...
        right = tk.LabelFrame(
            main,
            text="Загрузка сохранённых моделей",
            bg="#111",
            fg="#e94560",
            font=("Helvetica", 11, "bold"),
            padx=10,
    ...
    def _build_left_panel(self, parent):
        ...
        for key, name, default_val in settings:
            ...
            entry_var = tk.StringVar(
                self,
                value=f"{self.current_config.get(key, default_val):.3f}",
            )
            ...
            var = tk.DoubleVar(
                self,
                value=self.current_config.get(key, default_val),
            )
            ...
            tk.Scale(
                ...
                length=260,
                showvalue=0,
                command=lambda v, k=key: self._on_scale_change(k, v),
            ).pack(side="left", fill="x", expand=True)

    def _build_right_panel(self, parent):
        tk.Label(
            parent,
            text="Выберите модель и нажмите «Загрузить»",
            bg="#111",
            fg="#aaaaaa",
            font=("Helvetica", 9),
        ).pack(anchor="w")
    ...
    def load_selected_model(self):
        ...
        if not selection:
            messagebox.showinfo(
                "Custom AI",
                "Сначала выберите модель в списке.",
            )
            return
        ...
        Log.info(f"Загружена CustomAlg модель: {data.get('name')}")
...
class SelfLearningProgress(tk.Toplevel):
    def __init__(
        ...
        mode_text = "∞ (ручная остановка)" if iterations < 0 else str(iterations)
    ...
    def stop(self):
        ...
        self.stop_event.set()
        self.status.config(text="Остановка... ждём завершения текущей итерации")
        ...
        try:
            while True:
                ...
                if entry.get("__error__"):
                    ...
                    messagebox.showerror(
                        "Self-Learning Error",
                        entry.get("__error__", "Unknown error"),
                    )

                    return

                self.status.config(text=f"Итерация {entry.get('iter', '?')}")
...
class TetrisSetup:
    def __init__(self, parent, username="Guest", profile=None):
        ...
        self.self_learning_model = tk.StringVar(
            self.root,
            value=f"{self.current_user}_selfAI",
        )

        self.self_learning_tags = tk.StringVar(
            self.root,
            value="self-learning,trained,tetris",
        )

        self.teacher_student_tags = tk.StringVar(
            self.root,
            value="teacher-student,trained,tetris",
        )
    ...
    def setup_ui(self):
        ...
        tk.Entry(
            self.ml_container,
            textvariable=self.self_learning_model,
            bg="#222",
            fg="white",
            width=25,
        ).pack(side="left", padx=5)
        ...
        tk.Entry(
            self.ml_container,
            textvariable=self.self_learning_tags,
            bg="#222",
            fg="white",
            width=35,
        ).pack(side="left", padx=5)
        ...
        tk.Checkbutton(
            self.ml_container,
            text="Показывать геймплей бота",
            variable=self.show_gameplay_var,
            bg=self.colors["bg"],
            fg=self.colors["text"],
            selectcolor=self.colors["frame_bg"],
            font=("Helvetica", 10),
        ...
        tk.Checkbutton(
            self.ml_container,
            text="Начинать с нулевых весов",
            variable=self.start_from_zero_var,
            bg=self.colors["bg"],
            fg=self.colors["text"],
            selectcolor=self.colors["frame_bg"],
            font=("Helvetica", 10),
        ...
        tk.Entry(
            self.ts_container,
            textvariable=self.teacher_student_tags,
            bg="#222",
            fg="white",
            width=40,
        ).pack(side="left", padx=5)
        ...
        try:
            ...
            if settings["game_mode"] == "teacher_student" and game is not None:
                try:
                    ...
                    Log.error(f"Ошибка сохранения teacher-student модели: {e}")
    ...
    def run_self_learning(self, settings):
        if self.self_learning_running:
            messagebox.showinfo("Self-Learning", "Обучение уже запущено.")
            return
        ...
        try:
            ...
            default_name = (
                settings.get("self_learning_model_name")
                or f"{self.current_user}_selfAI"
            )
            ...
            model_name = ask_model_name(
                parent=self.root,
                title="Self-Learning",
                label="Имя модели для сохранения:",
                default_name=default_name,
            )

            if not model_name:
                Log.info("Self-learning отменён: пользователь не задал имя модели.")
                return
            ...
            def worker():
                try:
                    result = engine.run(
                        on_iter_callback=lambda entry: q.put(entry),
            ...
            self.self_learning_running = False
            Log.error(f"Ошибка self-learning: {e}")
    ...
    def _finish_self_learning(self, model_name: str, ai_type: str, settings: dict, result: dict):
        try:
            ...
            messagebox.showinfo(
                ...
                f"Модель сохранена:\n{model_name}\n\n"
                f"Выполнено итераций: {iterations_done}\n"
                f"Лучший счёт: {best_score}",
            )

        except Exception as e:
            Log.error(f"Ошибка сохранения self-learning модели: {e}")
    ...
    def save_teacher_student_model(self, game, settings):
        ...
        if teacher is None:
            Log.warning("Teacher-student: учитель не найден, модель не сохранена.")
            return
        ...
        if not model_name:
            Log.warning("Teacher-student: пользователь отменил сохранение модели.")
            return
        ...
        if student and getattr(student, "bot", None):
            if hasattr(student.bot, "export_learned_weights"):
                try:
                    ...
                    Log.error(f"Ошибка экспорта весов ученика: {e}")
        ...
        try:
            ...
            Log.info(f"🎓 Teacher-student модель сохранена: {model_name}")
...
class GameplayViewer(tk.Toplevel):
    def __init__(self, parent, width=15, height=30, cell_size=18):
        ...
        self.title("🧠 Self-Learning — Геймплей бота")
        self.configure(bg="#111")
        self.transient(parent)
    ...
    def update_frame(self, grid, score, lines, piece_name, placed_pieces):
        ...
        try:
            ...
            self.info_label.config(
                text=f"Ход: {placed_pieces} | Счёт: {score} | Линии: {lines} | Фигура: {piece_name}"
            )
...
class App:
    ...
    def show_main_menu(self):
        self.root.title("Tetris MP - Главное меню")
        self.root.geometry("400x350")
